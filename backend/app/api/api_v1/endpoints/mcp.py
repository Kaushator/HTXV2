...
from app.schemas.mcp import (MarketDataUpdate, PortfolioUpdate, SystemHealth,
                             TaskInfo, TaskRequest, TaskResponse,
...
async def get_tools():
    ...
    return {
        "websocket": {
            "endpoint": "/api/v1/mcp/ws",
            "auth": "JWT bearer in first message",
        },
        "endpoints": [
...
async def schedule_task(
    ...
    try:
        task_id = await mcp_service.schedule_task(
            task_name=task_request.task_name, parameters=task_request.parameters
        )
    ...
    try:
        ...
        if auth_data.get("type") != "auth":
            await websocket.send_text(
                json.dumps(
                    {"type": "error", "message": "First message must be authentication"}
                )
            )
        ...
        if not payload:
            await websocket.send_text(
                json.dumps({"type": "error", "message": "Invalid authentication token"})
            )
            await websocket.close()
            return
        ...
        if not user_id:
            await websocket.send_text(
                json.dumps({"type": "error", "message": "Invalid token payload"})
            )
            await websocket.close()
            return
        ...
        if not user or not user.is_active:
            await websocket.send_text(
                json.dumps({"type": "error", "message": "User not found or inactive"})
            )
            await websocket.close()
            return
