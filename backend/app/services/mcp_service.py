...
from app.schemas.mcp import (
    ...
    TaskInfo, TaskStatus, WebSocketMessage,
...
class MCPService:
    ...
    async def check_system_health(self) -> SystemHealth:
        ...
        if htx_health.status != HealthStatus.HEALTHY and overall_status == HealthStatus.HEALTHY:
            overall_status = HealthStatus.DEGRADED
        
        coingecko_health = await self._check_coingecko_api_health()
        services.append(coingecko_health)
        if coingecko_health.status != HealthStatus.HEALTHY and overall_status == HealthStatus.HEALTHY:
            overall_status = HealthStatus.DEGRADED
    ...
    async def _check_htx_api_health(self) -> ServiceHealth:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get("https://api.huobi.pro/v1/common/timestamp", timeout=5.0)
                response_time = (time.time() - start_time) * 1000
    ...
    async def _check_coingecko_api_health(self) -> ServiceHealth:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get("https://api.coingecko.com/api/v3/ping", timeout=5.0)
                response_time = (time.time() - start_time) * 1000
    ...
    async def schedule_task(self, task_name: str, parameters: Optional[Dict[str, Any]] = None) -> str:
