...
def test_imports():
    ...
    try:
        # Test core imports
        from app.api.api_v1.endpoints import data, websocket
        ...
        market_data = MarketDataResponse(
            symbol="BTCUSDT", price=50000.00, timestamp=datetime.utcnow()
        )
        print(
            f"✅ Market data schema works: {market_data.symbol} @ ${market_data.price}"
        )

        ws_message = WebSocketMessage(
            type="market_data", data={"test": "data"}, timestamp=datetime.utcnow()
        )
...
def test_file_upload_logic():
    ...
    try:
        ...
        for filename in valid_files:
            ...
            ), f"Extension {extension} should be allowed"
