...
# Конфигурация
$backupDir = ".$($pathSeparator)backups"
$timestamp = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
$pgBackupFile = "$backupDir$($pathSeparator)pg_backup_$timestamp.sql"
$redisBackupFile = "$backupDir$($pathSeparator)redis_backup_$timestamp.rdb"
$configBackupFile = "$backupDir$($pathSeparator)config_backup_$timestamp.zip"
...
function Backup-Data {
    ...
    $tempConfigDir = "$backupDir$($pathSeparator)temp_config_$timestamp"
    New-Item -Path $tempConfigDir -ItemType Directory | Out-Null

    $allFilesExist = $true
    foreach ($file in $configFiles) {
        $filePath = $file.Replace('/', $pathSeparator)
        if (Test-Path $filePath) {
            $destDir = Join-Path -Path $tempConfigDir -ChildPath (Split-Path -Parent $filePath)
            if (-not (Test-Path $destDir)) {
                New-Item -Path $destDir -ItemType Directory -Force | Out-Null
            }
            Copy-Item -Path $filePath -Destination (Join-Path -Path $tempConfigDir -ChildPath $filePath) -Force
        } else {
            Write-Host "   Файл $filePath не найден" -ForegroundColor $warningColor
            $allFilesExist = $false
        }
    }
    ...
    if ($isWindowsOS) {
        Compress-Archive -Path "$tempConfigDir$($pathSeparator)*" -DestinationPath $configBackupFile -Force
    } else {
        # В Linux используем zip
        zip -r $configBackupFile "$tempConfigDir$($pathSeparator)*" 2>/dev/null
    }
